...
def InterFace_data(ctrl_line, C1, C2):
    S = ctrl_line.split(", ")
    D = {}
...
def File_finder(File_name, line, sub_thread_count, thread_id, C1, C2, SameProduct, TargetRate, complaint_id, FILE_COUNT):
    ...
    while (1):
        ...
        if (ctrl_line):
            flag = 1
            if (SameProduct != -1):
                TargetRate_2 = TargetRate
                if (TargetRate_2 == 0.0):
                        TargetRate_2 = 100
                try:
                    if (similarity_rate(line.split(", ")[SameProduct], ctrl_line.split(", ")[SameProduct], 0, 0) < TargetRate_2):
                        flag = 0
                except:
                    flag = 0
            if (flag == 1):
                ...
                if (match > TargetRate):
                    ...
                    DATA.append(InterFace_data(ctrl_line, C1, C2))
        else:
            return 0

############     Create (100 / thread_count) sub-threds and send them to match check     ############
def	sub_thread_start(File_name, line, thread_id, C1, C2, SameProduct, TargetRate, complaint_id, FILE_COUNT):
    T = []
    for i in range(FILE_COUNT):
        t = threading.Thread(target = File_finder, args = (File_name, line, int(100 / FILE_COUNT), thread_id, C1, C2, SameProduct, TargetRate, complaint_id, FILE_COUNT))
        T.append(t)
        t.start()
    for j in T:
        t.join()
...
def thread_start(C1, C2, SameProduct, TargetRate, complaint_id, FILE_COUNT):
    ...
    for thread_id in range(FILE_COUNT):
        t = threading.Thread(target = sub_thread_start, args=("childFile{}".format(thread_id + 1), line, thread_id, C1, C2, SameProduct, TargetRate, complaint_id, FILE_COUNT))
        T.append(t)
...
def start_find(_C1, _C2, _SameProduct, _TargetRate, _complaint_id, _FILE_COUNT):
    ...
    thread_start(int(_C1), int(_C2), int(_SameProduct), float(_TargetRate), str(_complaint_id), int(_FILE_COUNT))
    ###	Threads runtime
