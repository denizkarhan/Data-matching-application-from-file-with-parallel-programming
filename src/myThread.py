...
def create_multi_file(file_size):
    file = open("../data/BigData", "r", encoding="utf-8")
    T = 0
    file_s = 1
...
def	sub_thread_start(File_name, line, thread_id, C1, C2, C3, C4, SameProduct, TargetRate, complaint_id, FILE_COUNT):
    ...
    for i in range(5):
        t = threading.Thread(target = File_finder, args = (File_name, line, int(1),
                            thread_id, C1, C2, C3, C4, SameProduct, TargetRate, complaint_id, FILE_COUNT))
        T.append(t)
        t.start()
...
def thread_start(C1, C2, C3, C4, SameProduct, TargetRate, complaint_id, FILE_COUNT):
    ...
    for thread_id in range(FILE_COUNT):
        t = threading.Thread(target = sub_thread_start, args=("childFile{}".format(thread_id + 1),
                            line, thread_id, C1, C2, C3, C4, SameProduct, TargetRate, complaint_id, FILE_COUNT))
        T.append(t)
